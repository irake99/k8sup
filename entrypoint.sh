...
echo "Running etcd"
etcdCID=$(docker run -d -v /usr/share/ca-certificates/:/etc/ssl/certs -v /var/lib/etcd:/var/lib/etcd -p 4001:4001 -p 2380:2380 -p 2379:2379 --restart=always --name k8sup-etcd quay.io/coreos/etcd /usr/local/bin/etcd --name etcd0  --advertise-client-urls http://$1:2379,http://$1:4001  --listen-client-urls http://0.0.0.0:2379,http://0.0.0.0:4001  --initial-advertise-peer-urls http://$1:2380  --listen-peer-urls http://0.0.0.0:2380  --initial-cluster-token etcd-cluster-1  --initial-cluster etcd0=http://$1:2380  --initial-cluster-state new --data-dir /var/lib/etcd)

echo "Setting etcd"
KERNEL_SHORT_VERSION=$(uname -r | cut -d '.' -f 1-2)
VXLAN=`echo "$KERNEL_SHORT_VERSION >= 3.9" | bc`
KERNEL_VXLAN=$(lsmod | grep vxlan)
if [ $VXLAN -eq 1 ] && [ ! -z "$KERNEL_VXLAN" ]; then
